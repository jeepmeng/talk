...
from dialog_service.dialog_pg import create_session, insert_message, get_history_by_session, get_session_user
...
async def start_dialog(data: StartDialogRequest):
    talk_id = await create_session(data.user_id, data.title)
...
async def ask_question(talk_id: int, data: AskRequest):
    ...
    print(type(session.get("create_by")))
    print(type(data.user_id))
    if not session or str(session.get("create_by")) != data.user_id:
    ...
    async def sse_stream_generator():
        ...
        # ✅ 写入数据库
        await insert_message(talk_id, int(data.user_id), data.question, response)
